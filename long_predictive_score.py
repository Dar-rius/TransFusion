...
def long_predictive_score_metrics(ori_data, generated_data):
    ...
    for itt in trange(iterations):
        generated_data_t = torch.from_numpy(generated_data).float()
        ...
        X_mb = torch.index_select(generated_data_t[:, :-1, : (dim - 1)], 0, train_idx)
        Y_mb = torch.index_select(generated_data_t[:, 1:, (dim - 1)], 0, train_idx)
    ...
    for i in range(no):
        with torch.no_grad():
            data_x = torch.from_numpy(X_mb[i]).float()
            data_y = torch.from_numpy(Y_mb[i]).float()
            pred_Y_curr = model(data_x.unsqueeze(0)).squeeze(0)
            MAE_temp = MAE_temp + mean_absolute_error(
                data_y.detach(), pred_Y_curr.detach()
            )
    total_count += len(Y_mb[i])
    predictive_score = MAE_temp / total_count
